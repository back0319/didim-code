...
![DidimCode 이진 탐색 문제 풀이와 AI 힌트](docs/assets/readme/didim-code-editor.png)

잘못된 제출의 판정과 실행 결과를 바탕으로, 정답 대신 다음 시도에서 확인할 핵심 힌트 하나를 제공합니다.

### 실행 단계 시각화

![DidimCode 코드 실행 단계 시각화](docs/assets/readme/didim-code-visualization.png)

현재 줄과 변수·조건·반복 상태를 같은 단계에서 연결해 코드가 실제로 실행되는 과정을 재생합니다.
